"""API для БАС Арена — сохранение сессий, начисление XP, управление профилем и достижениями"""
import json
import os
import psycopg2
...
ACHIEVEMENTS_CONFIG = [
    {"id": "first_blood",   "xp": 50,  "check": lambda s, stats: stats["total_sessions"] >= 1},
    {"id": "sharp_shooter", "xp": 150, "check": lambda s, stats: s.get("accuracy", 0) >= 90},
    {"id": "speed_demon",   "xp": 200, "check": lambda s, stats: s.get("score", 0) >= 500},
    {"id": "consistency",   "xp": 300, "check": lambda s, stats: stats["streak_days"] >= 7},
    {"id": "highscore",     "xp": 500, "check": lambda s, stats: s.get("score", 0) >= 1000},
    {"id": "grind",         "xp": 400, "check": lambda s, stats: stats["total_sessions"] >= 50},
    {"id": "perfect",       "xp": 750, "check": lambda s, stats: s.get("accuracy", 0) >= 100},
    {"id": "marathon",      "xp": 250, "check": lambda s, stats: stats["today_minutes"] >= 30},
]
...
def get_player_stats(cur, player_id: str) -> dict:
    cur.execute(f"""
        SELECT COUNT(*) as total,
               COALESCE(SUM(duration), 0) as today_min
...
def check_achievements(cur, player_id: str, session: dict, stats: dict) -> list:
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if method == "GET" and action == "sessions":
            player_id = qs.get("player_id", "ИГРОК_001")
            cur.execute(f"""
                SELECT id, score, duration, accuracy, level, xp_earned, played_at
                FROM {SCHEMA}.sessions
                WHERE player_id = '{player_id}'
            ...
            sessions = [{"id": r[0], "score": r[1], "duration": r[2], "accuracy": r[3],
                         "level": r[4], "xp_earned": r[5],
                         "date": r[6].strftime("%d %b") if r[6] else ""} for r in rows]
            conn.close()
        ...
        if method == "POST":
            ...
            cur.execute(f"""
                INSERT INTO {SCHEMA}.sessions (player_id, score, duration, accuracy, level, xp_earned)
                VALUES ('{player_id}', {score}, {duration}, {accuracy}, '{level}', {xp_earned})
            """)
            ...
            cur.execute(f"UPDATE {SCHEMA}.players SET xp = xp + {total_xp} WHERE player_id = '{player_id}'")
            cur.execute(f"SELECT xp FROM {SCHEMA}.players WHERE player_id = '{player_id}'")
            new_total_xp = cur.fetchone()[0]
            conn.commit()
            conn.close()
            return {"statusCode": 200, "headers": cors, "body": json.dumps({
                "xp_earned": xp_earned, "bonus_xp": bonus_xp,
                "total_xp": new_total_xp, "new_achievements": new_achs
            }, ensure_ascii=False)}

        # PUT ?action=profile — обновить аватар/никнейм
        if method == "PUT" and action == "profile":
            ...
            if nickname:
                sets.append(f"nickname = '{nickname}'")
            if sets:
